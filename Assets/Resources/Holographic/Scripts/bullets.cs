...
public class bullets : MonoBehaviour {
	...
	void drawBullets(){
		...
		for (int i=0;i<bullets_n;i++){
			...
			this.transform.Find("bullet"+i).renderer.material.color = new Color(1,1,1,.65f*a);
		}
		for (int i1=(int)bullets_n;i1<bullets_max;i1++){
			this.transform.Find("bullet"+i1).renderer.material.color = new Color(this.renderer.material.color.r,this.renderer.material.color.b,this.renderer.material.color.g,0);
		}
	}
